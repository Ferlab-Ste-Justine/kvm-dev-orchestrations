...
resource "null_resource" "starrocks_pkcs12_from_pem" {
  triggers = {
    cert    = sha256(local_file.starrocks_cert.content)
    key     = sha256(local_file.starrocks_key.content)
    passout = local.params.starrocks.ssl_keystore_password
    passin  = local.params.starrocks.ssl_key_password
  }
